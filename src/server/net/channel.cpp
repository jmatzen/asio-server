...
Channel::~Channel()
{
   spdlog::info("channel {} closed", usize(this));
}

void Channel::startRead_(std::function<void(const std::span<u8> &)> const &callback)
{
   ...
   socket_->async_read_some(buf, [=](const boost::system::error_code &error, std::size_t const bytes_transferred) {
