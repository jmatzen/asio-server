 ...
 public:
   void postConstruct() override
   {
      ...
      acceptor_ = std::make_unique<Acceptor>(8080, [=](auto &&s) {
         ...
         auto pipeline = std::make_shared<net::ChannelPipeline>(std::move(channel));
         pipeline->addLast(std::make_unique<net::DumpChannelHandler>(), "dump");
         pipeline->addLast(std::make_unique<net::Http2ChannelHandler>(), "http2");
      ...
      timer = AppContext::getContext().getIoContext().fixedDelay(boost::posix_time::millisec(1000), []() {
